...
def clustersgc_objective(space):
    clustering_machine = ClusteringMachine(MODEL, round(space['degree']), CLUSTER_METHOD,CLUSTER_NUM , TEST_RATIO, graph, features,
                                           labels)  # normalize
    precompute_time = clustering_machine.decompose()
    print("Finished decompose")
    gcn_trainer = ClusterTrainer(MODEL, LAYERS, DROPOUT, round(space['degree']), EPOCHS,LR,WEIGHT_DECAY,clustering_machine,plot=False)
    ...
    print('degree:{}'.format(round(space['degree'])) + 'f1score: {:.4f}'.format(test_f1))
    return {'loss': -test_f1, 'status': STATUS_OK}

trials = Trials()
best = fmin(clustersgc_objective, space=space, algo=tpe.suggest, max_evals=1,trials=trials)
#print("Best weight decay: {:.2e} lr:{}".format(best["weight_decay"],best['lr']))
print("Best degree:{}".format(best['degree']))
