...
def clustersgc_objective(space):
    clustering_machine = ClusteringMachine(MODEL, DEGREE, CLUSTER_METHOD,CLUSTER_NUM , space['test_ratio'], graph, features,
                                           labels)  # normalize
    precompute_time = clustering_machine.decompose()
    print("Finished decompose")
    gcn_trainer = ClusterTrainer(MODEL, LAYERS, DROPOUT, DEGREE, EPOCHS,LR,WEIGHT_DECAY,clustering_machine)
    ...
    print('test_ratio:{}'.format(space['test_ratio']) + 'f1score: {:.4f}'.format(test_f1))
    return {'loss': -test_f1, 'status': STATUS_OK}

trials = Trials()
best = fmin(clustersgc_objective, space=space, algo=tpe.suggest, max_evals=60,trials=trials)
#print("Best weight decay: {:.2e} lr:{}".format(best["weight_decay"],best['lr']))
print("Best test_ratio:{}".format(best['test_ratio']))
...
xs = [t['misc']['vals']['test_ratio'] for t in trials.trials]
ys = [t['result']['loss'] for t in trials.trials]
ax.scatter(xs, ys, s=20, linewidth=0.01, alpha=0.75)
ax.set_title('$val$ $vs$ $x$ ', fontsize=18)
ax.set_xlabel('$test_ratio$', fontsize=16)
