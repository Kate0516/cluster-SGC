...
space = {#'weight_decay' : hp.loguniform('weight_decay', log(1e-10), log(1e-4)),
         'lr' : hp.uniform('lr', 0.005, 0.5), #0.2for cora citerseer, 0.5for pubmed
...
def clustersgc_objective(space):
    gcn_trainer = ClusterTrainer(MODEL, LAYERS, DROPOUT, DEGREE, EPOCHS,space['lr'],WEIGHT_DECAY,clustering_machine)
    ...
    print('lr:{}'.format(space['lr']) + 'f1score: {:.4f}'.format(test_f1))
...
print("Best lr:{}".format(best['lr']))

os.makedirs("./cluster-tuning", exist_ok=True)
path = 'cluster-tuning/{}-lr.txt'.format(DATASET_NAME)
